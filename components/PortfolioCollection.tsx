...
export function PortfolioCollection({ collection, children }: { collection: "systems" | "product"; children?: ReactNode }) {
  ...
  function renderArtifact(id: Artifact["id"]) {
    ...
    const isWide = artifact.layout === "wide";

    return (
      <figure key={id} className={isWide ? "col-span-full min-w-0" : "min-w-0"}>
        <button type="button" onClick={() => open(index)} aria-label={`Enlarge ${artifact.title}`} title={`Enlarge ${artifact.title}`} className="group relative block w-full cursor-zoom-in overflow-hidden border border-ink/10 bg-white focus-visible:outline-2 focus-visible:outline-offset-4">
    ...
    <div className="w-full">
      ...
      {collectionSections.map((section) => (
        <section key={section.id} id={section.id} aria-labelledby={`${section.id}-title`} className="mt-12 scroll-mt-6 border-t border-ink/15 pt-6 sm:mt-16 sm:pt-7">
          ...
          </div> : <div className="mb-6 flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2">
      ...
      {!isProduct && (
      <section id="full-documents" aria-labelledby="full-documents-title" className="mt-12 scroll-mt-6 border-t border-ink/15 pt-6 sm:mt-16 sm:pt-7">
        <h3 id="full-documents-title" className="text-xl font-medium leading-7 text-ink">Full documents</h3>
        <div className="mt-5 divide-y divide-ink/10 border-b border-ink/10">
          {documents.map((doc) => (
            <div key={doc.file} className="flex items-center gap-3 py-3">
              <a href={`/documents/${doc.file}`} title={doc.description} target="_blank" rel="noopener noreferrer" className="min-w-0 flex-1 text-sm font-medium leading-6 text-ink underline decoration-ink/20 underline-offset-4 hover:decoration-ink">{doc.title}</a>
              <span className="shrink-0 font-mono text-[11px] text-graphite/60">{doc.pages} pages</span>
